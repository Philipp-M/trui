...
impl<T, A, C, VT: ViewSequence<T, C, A>> View<T, C, A> for LinearLayout<T, A, VT> {
    type State = VT::State;

    type Element = widget::LinearLayout;

    fn build(&self, cx: &mut Cx<C>) -> (Id, Self::State, Self::Element) {
...
pub fn h_stack<T, C, A, VT: ViewSequence<T, C, A>>(children: VT) -> LinearLayout<T, A, VT> {
    LinearLayout {
        children,
...
pub fn v_stack<T, C, A, VT: ViewSequence<T, C, A>>(children: VT) -> LinearLayout<T, A, VT> {
    LinearLayout {
        children,
